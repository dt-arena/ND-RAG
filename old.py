...
class ImprovedAutomatedTestGenerator:
    ...
    def _wait_for_rate_limit(self, estimated_tokens: int = 1000):
        ...
        if self.token_usage_per_minute + estimated_tokens > safe_token_limit:
            ...
            if sleep_time > 0:
                print(f"⚠️ Token limit approaching ({self.token_usage_per_minute}/{safe_token_limit:.0f}). Waiting {sleep_time:.2f} seconds...")
        ...
        if len(self.request_times) >= self.max_requests_per_minute:
            ...
            if sleep_time > 0:
                print(f"⚠️ Request limit reached ({len(self.request_times)}/{self.max_requests_per_minute}). Waiting {sleep_time:.2f} seconds...")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if not api_key:
                    ...
                    api_key = ""
                    logger.warning("Using hardcoded API key. Consider setting OPENAI_API_KEY environment variable.")
                ...
                if "rate_limit_exceeded" in error_str or "429" in error_str or "rate limit" in error_str.lower():
                    ...
                    print(f"\n⚠️ Rate limit error (attempt {attempt + 1}/{self.max_retries}):")
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        wait_message = "Rate limit exceeded after all retries. This API key may have very low rate limits (typical for free tier). Please wait 1-2 hours before trying again."
                        logger.error(wait_message)
                        print(f"\n❌ {wait_message}")
                        print(f"💡 Tip: Consider upgrading your OpenAI account for higher rate limits.")
...
{reset_handling}    }}
    ...
    def _find_class_by_signature(self, function_code: str) -> Optional[str]:
        ...
        if candidates:
            ...
            chosen = candidates[0]
            print(f"🔍 Signature match in '{chosen[1]}' → class '{chosen[0]}' (score {chosen[3]})")
            return chosen[0]
